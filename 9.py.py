...
class Villager:
    ...
    def draw(self):
        if self.alive:
            # Animate the limbs for walking effect
            self.animation_offset = math.sin(time.time() * self.animation_speed) * 5

            # Draw head using midpoint circle algorithm 
            midpoint_circle(self.x, self.y + self.head_radius, self.head_radius, (0.2, 0.8, 0.2))

            glColor3f(0.2, 0.8, 0.2)  # Green color for body and limbs
            glBegin(GL_POINTS)
            
            # Draw body 
            for dy in range(0, self.body_length):
                plot_point(self.x, self.y - dy)

            # Draw legs with animation
            leg_start_y = self.y - self.body_length
            # Left leg
            for i in range(self.limb_length):
                x = self.x - i * 0.7 + self.animation_offset * 0.3
                y = leg_start_y - i * 0.7
            ...
            # Left arm
            for i in range(self.limb_length):
                x = self.x - i * 0.7 - self.animation_offset * 0.2
                y = arm_start_y + i * 0.3
                plot_point(int(x), int(y))
            # Right arm
            for i in range(self.limb_length):
